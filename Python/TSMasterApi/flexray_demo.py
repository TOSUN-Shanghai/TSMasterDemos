...
LastEditTime: 2023-02-26 20:59:51
...
TSMasterAPI.initialize_lib_tsmaster_with_project(APPName,b'C:\\Users\\86576\\Desktop\\test')
...
TSignal_ = TSMasterAPI.tscom_flexray_get_signal_definition(b'0/BackboneFR/VGM/VgmBackBoneFr00/FRNetworkStatus')
# 该函数为回调事件 可以查看报文实时记录
def Flexray_RX(obj,AFlexray):
    '''
    回调事件 发送完成事件 接受事件 都在该函数中实现
    '''
    if(AFlexray.contents.FSlotId == 1 and AFlexray.contents.FDir == 1):
...
def Flexray_Pre(obj,AFlexray):
    ...
    '''
    if(AFlexray.contents.FSlotId == 1):
...
TSMasterAPI.tsdb_unload_flexray_dbs()
time.sleep(2)
ret = TSMasterAPI.tsdb_load_flexray_db(b'C:\\Users\\86576\\Desktop\\SDB21400_BGM3_High_RCM_BackboneFR_211224(1).xml',b'0,1',AID)
...
ret = TSMasterAPI.tscom_flexray_rbs_activate_cluster_by_name(1,True,b"BackboneFR",False)
ret = TSMasterAPI.tscom_flexray_rbs_activate_ecu_by_name(1,True,b"BackboneFR",b"VGM",True)
...
while 1:
    ...
    if key == 'q':
        ...
        if AValue>14:
            AValue = 0.0
        ret = TSMasterAPI.tscom_flexray_rbs_set_signal_value_by_address(b'1/BackboneFR/VGM/VgmBackBoneFr00/DstTrvldVldFromVehCom',ctypes.c_double(AValue))
        print('AValue = ',AValue)
        signal_value =  TSMasterAPI.tscom_flexray_rbs_get_signal_value_by_address(b'1/BackboneFR/VGM/VgmBackBoneFr00/DstTrvldVldFromVehCom')
