...
import time
import ctypes
from can.interfaces.libtosun import libtosunBus
...
class IsoTPSocketConnection(BaseConnection):
    ...
    def open(self):
        self.tpsock.bind(self.interface, rxid=self.rxid, txid=self.txid, *self.tpsock_bind_args,
                         **self.tpsock_bind_kwargs)
        self.exit_requested = False
        self.rxthread = threading.Thread(target=self.rxthread_task)
        self.rxthread.start()
        self.opened = True
...
class PythonIsoTpConnection(BaseConnection):
    ...
    def specific_send(self, payload):
        ...
        self.toIsoTPQueue.put(
            bytearray(payload))  # isotp.protocol.TransportLayer uses byte array. udsoncan is strict on bytes format
...
class J2534Connection(BaseConnection):
    ...
    def __init__(self, windll, rxid, txid, name=None, debug=False, *args, **kwargs):
        ...
        self.result, self.firmwareVersion, self.dllVersion, self.apiVersion = self.interface.PassThruReadVersion(
            self.devID)
        self.logger.info("J2534 FirmwareVersion: " + str(self.firmwareVersion.value) + ", dllVersoin: " + str(
            self.dllVersion.value) + ", apiVersion" + str(self.apiVersion.value))
...
class TOSUNConnection(BaseConnection):
    DLC_DATA_BYTE_CNT = (
        0, 1, 2, 3, 4, 5, 6, 7,
        8, 12, 16, 20, 24, 32, 48, 64
    )
    max_len = 4095
    AResdata = ctypes.create_string_buffer(max_len)
    AResponseDataSize = ctypes.c_uint32(len(AResdata))

    def __init__(self, bus, request_id, respond_id, channel=0, **kwargs):
        BaseConnection.__init__(self)
        if isinstance(bus, libtosunBus):
            self._bus = bus
            self._channel = channel
            self.__udsHandle = ctypes.c_int32(0)
            self.opened = False
            self.request_id = request_id
            self.respond_id = respond_id
            self.kwargs = kwargs
            self._receive_data = queue.Queue(maxsize=1)
            self.timeout = 0.1
            if 'timeout' in self.kwargs:
                self.timeout = int(self.kwargs['timeout']) if self.kwargs['timeout'] > 0 else 0.1
        else:
            raise "Trapping mistyped and unexpected elements and attributes "

    def open(self):
        self.fd = False
        if 'fd' in self.kwargs:
            self.fd = self.kwargs['fd']
        self.dlc = 8
        if 'dlc' in self.kwargs:
            try:
                self.dlc = self.DLC_DATA_BYTE_CNT.index(self.kwargs['dlc'])
            except:
                self.dlc = self.kwargs['dlc']
        self.is_std = True
        if 'is_std' in self.kwargs:
            self.is_std = self.kwargs['is_std']
        self.function_id = 0x7fe
        if 'function_id' in self.kwargs:
            self.function_id = self.kwargs['function_id']
        if 0 == self._bus.device.tsdiag_can_create(self.__udsHandle, self._channel, self.fd, self.dlc, self.request_id,
                                                   self.is_std, self.respond_id, self.is_std, self.function_id,
                                                   self.is_std,self.timeout) :
            self.opened = True
    ...
    def close(self):
        self._bus.device.tsdiag_can_delete(self.__udsHandle)
        self.opened = False
        self.logger.info('Fake Connection closed')

    def specific_send(self, payload, is_function=False):
        if is_function:
            self._bus.device.tstp_can_send_functional(self.__udsHandle, payload)
            return
        r, recv_data = self._bus.device.tstp_can_request_and_get_response(self.__udsHandle, payload)
        if r != 0:
            self.logger.error(self._bus.device.tscan_get_error_description(r))
            return
        if len(recv_data) > 0:
            if self._receive_data.full():
                self._receive_data.get()
            self._receive_data.put(recv_data)

    def specific_wait_frame(self, timeout=0.1):
        return self._receive_data.get() if not self._receive_data.empty() else b''

    def empty_rxqueue(self):
        while not self._receive_data.empty():
            self._receive_data.get()
